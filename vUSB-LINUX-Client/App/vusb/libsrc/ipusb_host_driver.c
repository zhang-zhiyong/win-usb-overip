...
#include "ipusb_host_driver.h"

#undef  PROGNAME
#define PROGNAME "libipusb"

struct ipusb_host_driver *host_driver;

#define SYSFS_OPEN_RETRIES 100

/* only the first interface value is true! */
static int32_t read_attr_ipusb_status(struct ipusb_usb_device *udev)
{
	 ...
	 *
	 * We may try and fail to open the ipusb_status of
	 ...
	 * ipusb_status to reappear.
	 */

	snprintf(attrpath, SYSFS_PATH_MAX, "%s/%s:%d.%d/ipusb_status",
	...
	if (retries == 0)
		dbg("ipusb_status not ready after %d retries",
		    SYSFS_OPEN_RETRIES);
	else if (retries < SYSFS_OPEN_RETRIES)
		dbg("warning: ipusb_status ready after %d retries",
...
static struct ipusb_exported_device *ipusb_exported_device_new(char *sdevpath)
{
	struct ipusb_exported_device *edev = NULL;
	size_t size;
	int i;
	...
	edev->status = read_attr_ipusb_status(&edev->udev);
	if (edev->status < 0)
		goto err;

	/* reallocate buffer to include usb interface data */
	size = sizeof(*edev) + edev->udev.bNumInterfaces *
		sizeof(struct ipusb_usb_interface);
...
static int refresh_exported_devices(void)
{
	...
	if (!suintf_list) {
		 ...
		 * the driver yet.
		 */
		dbg("bind " IPUSB_HOST_DRV_NAME ".ko to a usb device to be "
		    "exportable!");
		return 0;
	}
	...
	dlist_for_each_data(sudev_list, sudev, struct sysfs_device) {
		edev = ipusb_exported_device_new(sudev->path);
		if (!edev) {
			dbg("ipusb_exported_device_new failed");
			continue;
		}
...
static void ipusb_exported_device_delete(void *dev)
{
	struct ipusb_exported_device *edev = dev;
	sysfs_close_device(edev->sudev);
	free(dev);
}
...
int ipusb_host_driver_open(void)
{
	...
	host_driver->edev_list =
		dlist_new_with_delete(sizeof(struct ipusb_exported_device),
				      ipusb_exported_device_delete);
...
struct ipusb_exported_device *ipusb_host_get_device(int num)
{
	...
	dlist_for_each_data(dlist, edev, struct ipusb_exported_device) {
		if (num == cnt)
			return edev;
		else
			cnt++;
